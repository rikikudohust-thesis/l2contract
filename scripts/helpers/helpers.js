...
class ForgerTest {
  constructor(maxTx, maxL1Tx, nLevels, l2Contract, rollupDB, realVerifier) {
  ...
  async forgeBatch(l1Batch, l1TxUserArray, l1TxCoordiatorArray, l2txArray, log) {
    ...
    const currentQueue = await this.l2Contract.nextL1ToForgeQueue();
    const SCL1TxData = await this.l2Contract.mapL1TxQueue(currentQueue);
    ...
    for (const functionSC in this.l2Contract.interface.functions) {
      if (this.l2Contract.interface.functions[functionSC].name == "calculateInputTest") {
    ...
    if (implementCalculateInputTest)
      await expect(
        this.l2Contract.calculateInputTest(
          newLastIdx,
          newStateRoot,
          newExitRoot,
        ...
        .to.emit(this.l2Contract, "ReturnUint256")
        .withArgs(bb.getHashInputs());

    await expect(
      this.l2Contract.forgeBatch(
        newLastIdx,
        newStateRoot,
        newExitRoot,
        compressedL1CoordinatorTx,
    ...
    ).to.emit(this.l2Contract, "ForgeBatch")
...
async function l1UserTxCreateAccountDeposit(
  ...
  const lastQueue = await l2Contract.nextL1FillingQueue();

  const lastQueueBytes = await l2Contract.mapL1TxQueue(lastQueue);
  ...
  if (tokenID != 0) {
    if (!isERC20Permit) {
      ...
      await expect(
        hardhatTokenHermez.connect(wallet).approve(l2Contract.address, loadAmount)
      ).to.emit(hardhatTokenHermez, "Approval");

      // const gasCost = await l2Contract.estimateGas[
      ...
      await expect(
        l2Contract.connect(wallet).addL1Transaction(
          babyjub,
          fromIdx0,
          loadAmountF,
          amountF0,
        ...
        .to.emit(l2Contract, "L1UserTxEvent")
    ...
    let txRes;
    await expect(
      txRes = await l2Contract.connect(wallet).addL1Transaction(
        babyjub,
        fromIdx0,
        loadAmountF,
        amountF0,
...
async function l1CoordinatorTxEth(tokenID, babyjub, wallet, l2Contract, chainIdHex) {
  // equivalent L1 transaction:

  const flatSig = await txUtils.signBjjAuth(wallet, babyjub.slice(2), chainIdHex, l2Contract.address);
...
async function l1CoordinatorTxBjj(tokenID, babyjub, l2Contract) {
...
async function AddToken(
  ...
  const tokensAdded = await l2Contract.registerTokensCount();

  // Send data and amount
  await expect(l2Contract.connect(wallet).addToken(hardhatToken.address, data))
    .to.emit(l2Contract, "AddToken")
