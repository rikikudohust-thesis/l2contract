...
const { DEFAULT_PAGE_SIZE, BASE_API_URL, API_VERSION } = require("./constants.js");
const { isHermezEthereumAddress, isHermezBjjAddress } = require("./addresses.js");
 ...
 * @param {Number} fromItem - Item = require(where to start the request
...
function getForgerUrls(nextForgerUrls) {
  return nextForgerUrls.includes(baseApiUrl) ? nextForgerUrls : [...nextForgerUrls, baseApiUrl];
}

/**
 * Checks a list of responses = require(one same POST request to different coordinators
...
/**
 * Fetches the URLs of the next forgers = require(the /state API
...
async function postPoolTransaction(transaction, nextForgerUrls = [], axiosConfig = {}) {
  nextForgerUrls = nextForgerUrls.length === 0 ? await getNextForgerUrls() : nextForgerUrls;
  return Promise.all(
    getForgerUrls(nextForgerUrls).map((apiUrl) => {
 ...
 */
async function postCreateAccountAuthorization(
  hezEthereumAddress,
  bJJ,
  signature,
  nextForgerUrls = [],
  ...
  return Promise.all(
    getForgerUrls(nextForgerUrls).map((apiUrl) => {
      return axios
        .post(
          `${apiUrl}/${API_VERSION}/account-creation-authorization`,
          {
            hezEthereumAddress,
            bjj: bJJ,
            signature,
...
async function getCreateAccountAuthorization(hezEthereumAddress, axiosConfig = {}) {
  return extractJSON(
    axios.get(`${baseApiUrl}/${API_VERSION}/account-creation-authorization/${hezEthereumAddress}`, axiosConfig)
  );
}
