...
const { feeFactors, feeFactorsAsBigInts } = require("./fee-factors.js");
const { bufToHex, getTokenAmountBigInt } = require("./utils.js");
const { HermezCompressedAmount } = require("./hermez-compressed-amount.js");
const { getPoolTransactions, getAccount } = require("./api.js");
const {
  getAccountIndex,
  getEthereumAddress,
...
const { TxType, TxState } = require("./enums.js");
const { INTERNAL_ACCOUNT_ETH_ADDR } = require("./constants.js");
const { hasLinkedTransaction, addLinkedTransaction } = require("./atomic-utils.js");
...
function buildTxCompressedData(tx) {
  ...
  console.log(`from: ${tx.fromAccountIndex}`)
  console.log(`to: ${tx.toAccountIndex}`)
  console.log(`tokenId: ${tx.tokenId}`)
  console.log(`nonce: ${tx.nonce}`)
  console.log(`fee: ${tx.fee}`)
  console.log(`toBjjSign: ${tx.toBjjSign}`)
...
async function computeL2Transaction(tx, bjj, token) {
  ...
  const feeInScalar = Scalar.fromString(
    getTokenAmountBigInt(tx.fee.toFixed(token.decimals), token.decimals).toString()
  );
...
const _encodeTransaction = encodeTransaction;
const _buildTxCompressedData = buildTxCompressedData;
module.exports = {
  _encodeTransaction,
  getL1UserTxId,
  getL2TxId,
