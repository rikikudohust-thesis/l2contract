...
async function estimateDepositGasLimit(token, decompressedAmount, overrides, usePermit, signerData, providerUrl) {
  if (token.id === 0) {
    ...
    try {
      ...
      const estimatedTransferGasBigNumber = await tokenContract.estimateGas.transfer(
      ...
      return usePermit ? estimatedTransferGas + GAS_LIMIT_HIGH + GAS_PERMIT : estimatedTransferGas + GAS_LIMIT_HIGH;
    } catch (err) {
 ...
 */
async function estimateWithdrawGasLimit(
  token,
  merkleSiblingsLength,
  amount,
  overrides,
  signerData,
  providerUrl,
  ...
  const nonInstantGas =
    token.id === ETHER_TOKEN_ID ? NON_INSTANT_WITHDRAW_ETH_GAS_COST : NON_INSTANT_WITHDRAW_ERC20_GAS_COST;
  ...
  try {
    ...
    const estimatedTransferGasBigNumber = await tokenContract
      .connect(CONTRACT_ADDRESSES[ContractNames.Hermez])
      .estimateGas.transfer(address, amount, overrides);

    // 230k + Transfer cost + (31k * siblings.length) + non-instant cost offset
    return (
      GAS_LIMIT_WITHDRAW +
      Number(estimatedTransferGasBigNumber.toString()) +
      SIBLING_GAS_COST * merkleSiblingsLength +
      finalNonInstantGas
    );
  } catch (err) {
...
module.exports = { estimateDepositGasLimit, estimateWithdrawGasLimit, estimateWithdrawCircuitGasLimit };
