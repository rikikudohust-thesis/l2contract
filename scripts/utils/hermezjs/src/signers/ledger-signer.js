...
class LedgerSigner extends ethers.Signer {
   ...
   */
  getAddress() {
    return this.eth.getAddress(this.path).then(({ address }) => address);
  }
  ...
  signMessage(message) {
    ...
    return this.eth.signPersonalMessage(this.path, messageHex.slice(2)).then((result) => {
  ...
  signTransaction(transaction) {
    return ethers.utils.resolveProperties().then((tx) => {
      const unsignedTx = ethers.utils.serializeTransaction(tx).substr(2);

      return this.eth.signTransaction(this.path, unsignedTx).then(({ r, s, v }) => {
  ...
  static connect(provider, options) {
    return TransportWebUSB.create().then((transport) => new LedgerSigner(transport, provider, options));
  }
}
