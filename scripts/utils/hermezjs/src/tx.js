...
const { postPoolTransaction, postAtomicGroup, getAccounts, getAccount } = require("./api.js");
const { HermezCompressedAmount } = require("./hermez-compressed-amount.js");
const {
  ContractNames,
  CONTRACT_ADDRESSES,
  GAS_LIMIT_LOW,
...
const { approve, isPermitSupported, permit } = require("./tokens.js");
const { getEthereumAddress, getAccountIndex } = require("./addresses.js");
const { getContract } = require("./contracts.js");
const { getProvider } = require("./providers.js");
const { generateL2Transaction } = require("./tx-utils.js");
const ERC20ABI = require("./abis/ERC20ABI.js");
const HermezABI = require("./abis/HermezABI.js");
const WithdrawalDelayerABI = require("./abis/WithdrawalDelayerABI.js");
const { SignerType } = require("./signers.js");
const { buildZkInputWithdraw, buildProofContract } = require("./withdraw-utils.js");
const { estimateDepositGasLimit, estimateWithdrawGasLimit } = require("./tx-fees.js");
const { generateAtomicGroup } = require("./atomic-utils.js");
 ...
 */
const deposit = async (amount, hezEthereumAddress, token, babyJubJub, signerData, providerUrl, gasLimit) => {
  ...
  const accounts = await getAccounts(hezEthereumAddress, [token.id]).catch(() => undefined);
  ...
  const permitSignature = usePermit
    ? await permit(
        fromTokenContract,
        ethereumAddress,
        CONTRACT_ADDRESSES[ContractNames.Hermez],
        decompressedAmount,
        signerData,
        providerUrl
      )
 ...
 */
const forceExit = async (amount, accountIndex, token, signerData, providerUrl, gasLimit) => {
  if (!HermezCompressedAmount.isHermezCompressedAmount(amount)) {
    throw new Error(
      "The parameter needs to be an instance of HermezCompressedAmount created with HermezCompressedAmount.compressAmount"
    );
  ...
  const account = await getAccount(accountIndex).catch(() => {
  ...
  const transactionParameters = [0, getAccountIndex(accountIndex), 0, amount.value, token.id, 1, "0x"];
 ...
 * @returns {Promise} transaction parameters
 */
const delayedWithdraw = async (hezEthereumAddress, token, signerData, providerUrl, gasLimit) => {
  const ethereumAddress = getEthereumAddress(hezEthereumAddress);
  const txSignerData = signerData || { type: SignerType.JSON_RPC, addressOrIndex: ethereumAddress };
  const delayedWithdrawalContract = getContract(
    CONTRACT_ADDRESSES[ContractNames.WithdrawalDelayer],
  ...
  const transactionParameters = [ethereumAddress, token.id === 0 ? ETHER_ADDRESS : token.ethereumAddress];
