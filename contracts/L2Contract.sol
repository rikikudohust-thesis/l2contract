...
contract ZkPayment is Helpers {
  ...
  event InitializezkPaymentEvent(uint8 forgeL1L2BatchTimeout, uint256 feeAddToken, uint64 withdrawalDelay);
  ...
  function forgeBatch(
    ...
    require(msg.sender == tx.origin, 'zkPayment::forgeBatch: INTENAL_TX_NOT_ALLOWED');
  ...
  function addL1Transaction(
    ...
    // check tokenID
    require(tokenID < tokenList.length, 'zkPayment::addL1Transaction: TOKEN_NOT_REGISTERED');

    // check loadAmount
    uint256 loadAmount = _float2Fix(loadAmountF);
    require(loadAmount < _LIMIT_LOAD_AMOUNT, 'zkPayment::addL1Transaction: LOADAMOUNT_EXCEED_LIMIT');

    // deposit token or ether
    if (loadAmount > 0) {
      if (tokenID == 0) {
        require(loadAmount == msg.value, 'zkPayment::addL1Transaction: LOADAMOUNT_ETH_DOES_NOT_MATCH');
      } else {
        require(msg.value == 0, 'zkPayment::addL1Transaction: MSG_VALUE_NOT_EQUAL_0');
  ...
  function _addL1Transaction(
    ...
    require(amount < _LIMIT_L2TRANSFER_AMOUNT, 'zkPayment::_addL1Transaction: AMOUNT_EXCEED_LIMIT');

    // toIdx can be: 0, _EXIT_IDX or (toIdx > _RESERVED_IDX)
    if (toIdx == 0) {
      require((amount == 0), 'zkPayment::_addL1Transaction: AMOUNT_MUST_BE_0_IF_NOT_TRANSFER');
    } else {
      if ((toIdx == _EXIT_IDX)) {
        require((loadAmountF == 0), 'zkPayment::_addL1Transaction: LOADAMOUNT_MUST_BE_0_IF_EXIT');
      } else {
        require(((toIdx > _RESERVED_IDX) && (toIdx <= lastIdx)), 'zkPayment::_addL1Transaction: INVALID_TOIDX');
      }
    }
    // fromIdx can be: 0 if create account or (fromIdx > _RESERVED_IDX)
    if (fromIdx == 0) {
      require(babyPubKey != 0, 'zkPayment::_addL1Transaction: INVALID_CREATE_ACCOUNT_WITH_NO_BABYJUB');
    } else {
      require((fromIdx > _RESERVED_IDX) && (fromIdx <= lastIdx), 'zkPayment::_addL1Transaction: INVALID_FROMIDX');
      require(babyPubKey == 0, 'zkPayment::_addL1Transaction: BABYJUB_MUST_BE_0_IF_NOT_CREATE_ACCOUNT');
    }
  ...
  function withdrawMerkleProof(
    ...
    if (permit.length == 0) {
      ...
      require(exitNullifierMap[numExitRoot][idx] == false, 'zkPayment::withdrawMerkleProof: WITHDRAW_ALREADY_DONE');
  ...
  function withdrawCircuit(
    ...
    require(exitNullifierMap[numExitRoot][idx] == false, 'zkPayment::withdrawCircuit: WITHDRAW_ALREADY_DONE');

    // get exit root given its index depth
    uint256 exitRoot = exitRootsMap[numExitRoot];

    uint256 input = uint256(sha256(abi.encodePacked(exitRoot, msg.sender, tokenID, amount, idx))) % _RFIELD;
    // verify zk-snark circuit
    require(
      withdrawVerifier.verifyProof(proofA, proofB, proofC, [input]) == true,
  ...
  function addToken(address tokenAddress) public {
    require(IERC20(tokenAddress).totalSupply() > 0, 'zkPayment::addToken: TOTAL_SUPPLY_ZERO');
    uint256 currentTokens = tokenList.length;
    require(currentTokens < _LIMIT_TOKENS, 'zkPayment::addToken: TOKEN_LIST_FULL');
    require(tokenAddress != address(0), 'zkPayment::addToken: ADDRESS_0_INVALID');
    require(tokenMap[tokenAddress] == 0, 'zkPayment::addToken: ALREADY_ADDED');
   ...
   */
  function _initializeVerifiers(address[] memory _verifiers, uint256[] memory _verifiersParams) internal {
  ...
  function _buildL1Data(uint256 ptr, bool l1Batch) internal view {
    ...
    require(l1UserLength + l1CoordinatorLength <= _MAX_L1_TX, 'zkPayment::_buildL1Data: L1_TX_OVERFLOW');
    ...
    for (uint256 i = 0; i < l1CoordinatorLength; i++) {
      ...
      require(tokenID < tokenList.length, 'zkPayment::_buildL1Data: TOKEN_NOT_REGISTERED');
  ...
  function _constructCircuitInput(
    ...
    uint256 l1L2TxsDataLength = ((rollupVerifiers[verifierIdx].nLevels / 8) * 2 + 5 + 1) *
      rollupVerifiers[verifierIdx].maxTx;

    // [(nLevels / 8) bytes]
    uint256 feeIdxCoordinatorLength = (rollupVerifiers[verifierIdx].nLevels / 8) * 4;
    ...
    require(dLen <= l1L2TxsDataLength, 'zkPayment::_constructCircuitInput: L2_TX_OVERFLOW');
    assembly {
    ...
    require(dLen <= feeIdxCoordinatorLength, 'zkPayment::_constructCircuitInput: INVALID_FEEIDXCOORDINATOR_LENGTH');
    assembly {
      calldatacopy(ptr, dPtr, dLen)
    }
    ptr += dLen;
